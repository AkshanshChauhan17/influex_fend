...
import { ArrowLeft, ArrowRight, Copy, Loader, X } from "react-feather";

export default function RegInfluencer() {
    ...
    const [formData, setFormData] = useState({
      ...
      const handleCopy = () => {
        navigator.clipboard.writeText(regStatus.login_key)
          ...
          .catch(err => {
            setIsCopy({visible: true, message: <span className="h-center">Failed to copy text:  {err} <X className="control-button" onClick={()=>setIsCopy({visible: false, message: "", class: ""})} /></span>, class: "error"});
          });
      };
      ...
      return (
        <div className="form-ar">
            ...
            <Link className="link-tag" to={"/"}><ArrowLeft size={20} /> Back</Link>
            <p className="sub-heading color-primary">Register as Influencer</p>
